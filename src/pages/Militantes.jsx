...
import { listarMilitantes, crearMilitante, actualizarMilitante, CONDICIONES, labelCondicion } from '../lib/militantes.js'
import { listarGruposBase, listarEtiquetas } from '../lib/catalogos.js'
...
export default function Militantes() {
  ...
  async function cargar() {
    ...
    setError(null)
    try {
      const [m, gb, et, pg] = await Promise.all([
        listarMilitantes(),
        listarGruposBase(),
        listarEtiquetas(),
        listarPagos(),
  ...
  // ── Set de ids que pagaron el mes en curso ──
  ...
  function limpiarFiltros() {
    setBusqueda(''); setFCondicion(''); setFEstado(''); setFGrupoBase(''); setFEtiqueta('')
  }

  async function guardarMilitante(datos, id) {
    try {
      if (id) {
        await actualizarMilitante(id, datos)
        mostrarToast('Cambios guardados')
      } else {
        await crearMilitante(datos)
  ...
  return (
    <div>
      ...
      <div className="fila-badges">
        <span className="badge gris">{militantes.length} registrados</span>
      ...
      <div className="barra-buscar">
        <input
          type="text"
          placeholder="Buscar militante o simpatizante…"
          value={busqueda}
            ...
            return (
              <div key={m.id} className="fila-militante" style={{ borderLeftColor: colorInd }}
                ...
                <div className="avatar" style={{
                  background: hexToRgba(colorInd, 0.18),
                  borderColor: colorInd, color: colorInd,
                }}>
                  {iniciales(m.nombre)}
                </div>
